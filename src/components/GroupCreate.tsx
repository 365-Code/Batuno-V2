...
const GroupCreate = ({newGroup, setNewGroup}: {newGroup: boolean, setNewGroup: any}) => {
  ...
  const [groupData, setGroupData] = useState({
    name: '',
    avatar: "https://img.freepik.com/free-vector/students-thinking-concept-illustration_114360-18754.jpg?w=740&t=st=1707023400~exp=1707024000~hmac=957ede2326f8b079f2fa96e8fe28ec43723ed57ef099f278c65ca160581c3f87"
  })
  ...
  return (
    <section className={`${newGroup ? "block" : "hidden"} sm:min-w-[350px] sm:block md:max-w-[500px] bg-green-50 shadow-sm shadow-black/30 flex-1 dark:bg-[#0d121b] dark:border-r dark:border-white/10`}>
      <div className="flex items-center gap-2">
        ...
        <input
          ...
          onChange={(e: ChangeEvent<HTMLInputElement>) => setGroupData((preVal) => ({...preVal, name: e.target.value}))}
      ...
      <ErrorText text={`Missing group ${error.contacts ? "contacts" : "name"}`} show={error.contacts || error.name} />
      </div>
      <div className="py-2 space-y-2 flex flex-col max-h-[94%]">
        <div className="flex justify-between items-center">
            <p>
          Add Contacts{" "}
          <span className="text-slate-400">You{newMembers.length > 0 && "+"+(newMembers.length)}</span>{" "}
            </p>
